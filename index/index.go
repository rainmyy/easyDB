...
type IRverseIndex interface {
	Add(doc Document)
	Delete(IntId uint64, keyword *Keyword)
...
func (index *SkipListInvertedIndex) Delete(IntId uint64, keyword *Keyword) {
	key := keyword.ToString()
	lock := index.getLock(key)
	lock.Lock()
	defer lock.Unlock()
...
func (index *SkipListInvertedIndex) searchQuery(query *TermQuery, onFlag uint64, offFlag uint64, orFlags []uint64) *skiplist.SkipList {
	switch {
...
func (index SkipListInvertedIndex) InersectionList(lists ...*skiplist.SkipList) *skiplist.SkipList {
	if len(lists) == 0 {
		return nil
	}
	if len(lists) == 1 {
